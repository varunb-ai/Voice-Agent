...
from agents.voice.prompts import build_system_prompt, build_greeting, build_closing
...
class VoiceBrain:
    ...
    def _llm_turn(self) -> Optional[str]:
        ...
        try:
            ...
            if is_rate_limit or is_timeout:
                ...
                resp = _gc.chat.completions.create(
                    ...
                    messages    = [{"role": "system", "content": compact_sys}] + self._messages[-8:],
                    tools       = TOOL_SCHEMAS,
                    temperature = 0.05,
                    max_tokens  = 120,
                    timeout     = 4.0,
        ...
        if msg.tool_calls:
            calls = [
                (c.function.name, _safe_json(c.function.arguments or "{}"))
